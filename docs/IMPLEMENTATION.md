...
`AnalysisWorkspaceController` now consumes dataset preference defaults together with application-owned sparse overrides and exposes effective preferences plus ranked results as shared `StateFlow` state. The existing SearchPane remains temporarily in place, but analysis input changes schedule debounced ranked recalculation and stale generations are prevented from replacing newer results. Legacy packages with no enabled ranking defaults and no effective hard/radius constraint retain explicit refresh instead of triggering an automatic unbounded candidate scan.
...
- Shared preference scoring, dataset-provided defaults, user overrides, and ranked analysis are now connected through `AnalysisWorkspaceController`; the current form-oriented SearchPane remains temporary until the map-first Desktop composition is implemented. Center-settlement lookup separately uses exact/prefix/substring/fuzzy alias ranking.
