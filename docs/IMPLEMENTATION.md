...
| `mobile/core` | Immutable domain/runtime models and pure geographic calculations | [`../mobile/IMPLEMENTATION.md`](../mobile/IMPLEMENTATION.md) |
| `mobile/application` | Headless dataset/search/analysis/preferences/workspace/settings/notebook/external contracts and logic |
| `mobile/presentation` | Deterministic platform-independent localization, formatting, summaries, and score explanations |
| `mobile/shared` | Shared Compose UI plus renderer-neutral map/runtime composition and GeoJSON overlays | [`../mobile/IMPLEMENTATION.md`](../mobile/IMPLEMENTATION.md) |
...
## Preference calibration reporting

`scripts/preference_calibration_report.py` is the cross-project calibration-evidence tool for the fixed dataset-provided Preference defaults. It reads a generated `georisk.sqlite` read-only, consumes enabled `metric_preference_default` rows plus `metric_definition` grouping and `settlement_metric` values, and reproduces the same LOWER/HIGHER piecewise-linear quality and weighted-score formulas used by shared Kotlin.

The report does not alter datasets or application state. It exposes metric known-value coverage and percentiles, endpoint saturation, enabled weight by persisted conceptual group, and aggregate score/weighted-coverage percentiles. Its default 90% endpoint-saturation threshold is a review warning only; product thresholds remain explicit configuration and are never adapted from percentiles at runtime. `make preference-calibration` points to the generated Belarus package by default and can be redirected to another representative package.
