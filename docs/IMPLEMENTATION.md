...
- Dataset-scoped favorite settlements are persisted in the application settings database and survive restart. Desktop supports transient multi-selection/select-all and explicit selected-Favorites transfer into the activated imported candidate source for re-analysis. Notes, frozen analysis snapshots, named saved searches, and comparisons are not implemented yet.
