...
`SearchPane.kt` remains the responsive compatibility surface used by Android and narrow/fallback layouts. It still receives persisted `MetricDefinition` values and creates min/max hard-filter rows generically. All definitions with `defaultEnabled = true` enter the initial filter state, empty min/max rows have no analytical effect, and **Add filter** exposes additional definitions. `FilterSummaryBuilder` continues to describe the structured hard request deterministically.
...
- Shared preference scoring, dataset defaults, user overrides, ranked analysis, and the map-first wide Desktop workspace are connected through `AnalysisWorkspaceController`; map-marker-originated selection, map-driven center selection, and final camera-padding/interaction polish remain pending. Center-settlement lookup separately uses exact/prefix/substring/fuzzy alias ranking.
