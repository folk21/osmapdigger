...
Mutable application state is separate from generated dataset SQLite. Shared `UserPreferencesRepository` and restore logic persist the current dataset ID, stable center ID, optional radius, dynamic metric ranges, sparse dataset-scoped preference overrides, and candidate-source state. Candidate-source persistence separates the active dataset/imported scope from an optional retained reviewed import containing stable IDs plus the original user text for editing; the text is never used as identity. Preference overrides may replace `enabled`, `target`, `limit`, or `weight` by stable metric ID while score direction remains owned by the dataset-provided `MetricPreferenceDefault`. Shared `MetricPreferenceOverrideResolver` layers valid user changes over the opened dataset defaults and ignores overrides for metrics no longer present in that default catalog. The same application-owned settings SQLite also stores enabled external-search provider definitions behind `ExternalSearchProviderRepository`. Desktop stores this database in `~/.osmapdigger/settings/preferences.sqlite`; Android uses app-private `filesDir/settings/preferences.sqlite`. The legacy `preferences.sqlite` filename is retained to avoid moving existing user state even though the database now owns broader application settings. The settings schema is version 6 and its lifecycle remains independent from `geo-format`. Version 4 added the persisted reviewed candidate-scope payload; version 5 added normalized `favorite_settlement` rows; version 6 adds optional favorite notes and the separate `favorite_analysis_snapshot` table with one versioned frozen snapshot per saved settlement.

## Desktop analysis UI and responsive filter UI

`DesktopAnalysisWorkspace.kt` is the current wide Desktop composition. It keeps the map primary, exposes a resizable analysis sidebar that defaults to roughly one third of the window, separates generic Required hard constraints from generic Preferences, and renders score/data-coverage-ranked results. Ranked result presentation is split into `RankedResultsUi.kt`: each card retains the numeric score, adds a 0–100 score indicator plus deterministic strongest/weakest contribution cues, and exposes persistent favorite membership independently from the single map/details selection. `FavoritesUi.kt` provides a Desktop notebook pane for compact settlement/current-analysis context, frozen saved score/criteria/preferences, inline note editing, explicit snapshot refresh, transient multi-selection/select-all, browsing/opening/removing/clearing saved settlements, and an explicit stable-ID transfer that replaces and activates the imported candidate list before returning to ranked results. Desktop transient UI never overlaps the platform map rectangle: the Add filter/import pickers occupy only the left analysis pane, while selected-settlement details occupy a dedicated lower pane beneath the map. Preference rows are generated from persisted defaults/definitions and edit only generic enabled/target/limit/weight values; no metric IDs are hardcoded in Compose.

`SearchPane.kt` remains the responsive compatibility surface used by Android and narrow/fallback layouts. It still receives persisted `MetricDefinition` values and creates hard-filter rows generically. `MetricFilterPresentationBuilder` derives human-facing controls only from persisted `measureType`, title, description, and unit metadata: distance uses Min/Max distance, count uses Min/Max number without a misleading `count` field suffix, coverage uses Min/Max coverage, and unknown future measure types fall back to generic Min/Max value controls. The Desktop full-pane Add filter chooser and responsive dropdown use the same semantic presentation contract. All definitions with `defaultEnabled = true` enter the initial filter state, empty ranges have no analytical effect, and **Add filter** exposes additional definitions. `FilterSummaryBuilder` continues to describe the structured hard request deterministically.
...
`ExternalSearchProviderRepository` returns enabled global and dataset-country providers from application settings SQLite. The packaged seed catalog is owned by `mobile/config/external-search-providers.json` and currently defines Google/Yandex plus country-specific property portals such as Kufar, Avito, Idealista, ImmobilienScout24, SeLoger, Rightmove, Funda, and Immoweb. Seeding uses `INSERT OR IGNORE`, so customized rows remain authoritative. `ExternalSearchUrlBuilder` expands persisted URL templates with percent-encoded settlement/query terms. `ExternalSearchBatchBuilder` reuses the same catalog for selected Favorites: only templates expressible through `{query}` without a required `{settlement}` placeholder are eligible, names become quoted `OR` alternatives, and deterministic chunks keep each final encoded URL within a conservative bound. Desktop renders the generated chunks as explicit actions and never opens multiple browser searches automatically. No property portal is scraped or embedded.
...
- Dataset-scoped Favorites survive restart and may carry an optional note plus one immutable versioned analysis snapshot. Desktop supports compact saved-context presentation, explicit snapshot refresh, transient multi-selection/select-all, and selected-Favorites transfer into the activated imported candidate source for re-analysis. Named saved searches, multiple historical snapshots, and comparison dashboards are not implemented yet.
