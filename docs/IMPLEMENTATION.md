...
The current Compose workflow does not consume these defaults yet, and user-customized preference persistence remains application-owned follow-up work in the active Desktop analysis sub-spec.
...
- A shared preference-scoring/ranked-analysis path and dataset-provided preference-default contract now exist, but the current Compose workflow still uses the legacy hard-filter search path and does not yet initialize ranking from those defaults; center-settlement lookup separately uses exact/prefix/substring/fuzzy alias ranking.
