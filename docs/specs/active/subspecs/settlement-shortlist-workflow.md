...
8. **Calibration validation tooling implemented; representative evidence pending** — `scripts/preference_calibration_report.py` reads the published runtime SQLite contract, reports enabled group weights, metric known-value/percentile and endpoint-saturation distributions, and overall score/coverage percentiles without changing runtime scoring or defaults. `make preference-calibration` targets the generated Belarus package by default. Representative Andorra/Belarus reports still must be recorded before deliberately changing checked-in `balanced-living` defaults or accepting this sub-spec.
