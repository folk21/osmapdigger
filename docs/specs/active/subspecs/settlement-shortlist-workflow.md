...
Architecture-hardening iterations 1–5 established dependency direction, typed failures, shared persistence/query ownership, and source-responsibility decomposition used by this workflow. Gate 6 now physically extracts immutable domain/geography code into `:core`; later feature/presentation module gates remain active and are interleaved only between completed, testable product increments.
