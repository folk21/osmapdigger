...
9. Scan ranked cards using the numeric score and the matching 0–100 horizontal score bar. Incomplete data coverage remains textual, and compact strongest/weakest cues summarize the existing deterministic contribution model. Use the independent **Shortlist** checkbox to mark several candidates without changing map/details focus; the shortlist count and **Clear shortlist** action appear in the results heading.
