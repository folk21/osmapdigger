...
Candidate-source core/workflow, visual ranked affordances, persistent dataset-scoped Favorites, explicit selected-Favorites transfer back into imported candidates, notebook notes/frozen analysis snapshots, explicit bounded batch external search, and portable deterministic export/share are implemented. Deterministic calibration-report tooling now reads generated runtime databases; representative Andorra/Belarus calibration evidence and configured/manual acceptance remain before archival.
