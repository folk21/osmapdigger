...
Candidate-source core/workflow, visual ranked affordances, persistent dataset-scoped Favorites, explicit selected-Favorites transfer back into imported candidates, notebook notes/frozen analysis snapshots, and explicit bounded batch external search are implemented; portable export/share remains the next product increment.
