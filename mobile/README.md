...
| `dataset` | Read-only analytical dataset boundary | `domain` | `GeoRepository` |
| `map` | Renderer-neutral map data/assets and overlay serialization | `domain` | `MapPackage`, `MapOverlayGeoJson` |
| `external` | External-search provider model, URL expansion, explicit platform link action | `domain` | `ExternalSearchProviderRepository`, `ExternalSearchUrlBuilder`, `ExternalLinkOpener` |
| `search` | Deterministic hard search, radius semantics, settlement-name matching | `domain`, `geo`, `dataset` | `SearchService`, `SettlementSearchService`, `SearchRequestSemantics` |
| `analysis` | Pure preference scoring, contributions, ranked-analysis orchestration | `domain`, `search`, `dataset` | `PreferenceScorer`, `SettlementRanker`, `SettlementAnalysisService` |
| `preferences` | Application-owned persisted search/preference models and restore/override logic | `domain`, `analysis` | `UserPreferencesRepository`, `MetricPreferenceOverrideResolver` |
| `presentation` | Deterministic formatting and human-readable analysis/search presentation | `domain`, `analysis`, `preferences` | `FilterSummaryBuilder`, `ScoreExplanationBuilder`, metric/localization presentation |
| `workspace` | Application orchestration for one opened analysis workspace | `domain`, `analysis`, `preferences`, `dataset` | `AnalysisWorkspaceController` |
...
| `ui` | Shared Compose presentation and feature composition | `domain`, `analysis`, `preferences`, `presentation`, `search`, `workspace`, `runtime`, `map`, `external` | `OsmapDiggerApp`, `DesktopAnalysisWorkspace`, `SearchPane`, `PlatformMapSurface` |
