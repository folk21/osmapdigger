...
`workspace/AnalysisWorkspaceController` connects `MetricPreferenceDefault`, saved overrides, `SettlementAnalysisService`, and `UserPreferencesRepository` into the active application flow. It also carries the current `SettlementCandidateScope`; an imported scope is persisted with the dataset-scoped user context and counts as an explicit bounded analysis constraint, so even a legacy dataset with no preferences may analyze that reviewed ID set without enabling the protected unbounded auto-scan. `SettlementShortlist` is separate transient stable-ID state owned by the workspace: add/remove/clear operations neither persist settings nor trigger ranked recalculation, and current ranked order is authoritative when visible shortlisted results are enumerated. Input changes schedule a 250 ms debounced recalculation and late stale generations cannot replace newer ranked results. The controller also exposes explicit generic preference-edit operations for enabled state, weight, target/limit thresholds, and reset-to-dataset-default while keeping persisted overrides sparse. Legacy packages with no enabled preference defaults and no effective hard/radius constraint do not trigger an automatic unbounded candidate scan; explicit refresh remains available through the compatibility SearchPane path.
...
The shared application settings schema remains version 4. Version 3 added `user_preferences.preferences_json`; version 4 added `candidate_scope_json`. Candidate-source payload version 2 now stores the active source plus an optional retained reviewed import (stable IDs and original source text) in that same column, so no additional SQLite migration is needed. The decoder remains compatible with version-1 candidate-scope payloads and reconstructs editable text from current canonical names when an older active import has no retained source text. The schema remains application-owned without changing `geo-format/VERSION`.
...
`shared/src/desktopMain/.../MapPanel.desktop.kt` is the default Desktop `actual` map surface. It renders the local package style, result/selection GeoJSON overlays, camera focus, attribution, normal settlement-marker activation, and optional WGS84 map-location activation on MapLibre Compose native hosts, and otherwise provides the non-fatal fallback. Shared UI can accept a small `PlatformMapSurface` override from the Desktop application host; the contract exposes only stable settlement IDs and `GeoPoint`, never MapLibre/JCEF types.

`desktopApp/build.gradle.kts` owns the platform-native runtime selection for macOS Apple Silicon Metal, Linux x86-64 OpenGL, and Windows x86-64 OpenGL. Intel macOS receives no incompatible MapLibre JNI runtime; instead `desktopApp/.../map/IntelMacWebMapSurface.kt` owns the JCEF + MapLibre GL JS renderer. `LocalWebMapServer` binds to loopback only, serves packaged browser assets, converts local PMTiles reads into XYZ vector-tile responses, and accepts bounded same-origin POST payloads for normal stable-ID marker activation and WGS84 map-location activation. `IntelMacWebMapSurface` marshals those callbacks onto the Desktop AWT event queue before invoking shared UI callbacks, keeping browser/native lifecycle outside shared code.
...
- transient shortlist selection exists for the current analysis session, but notebook/favorites, named saved searches, notes, and comparison state are not persisted yet.
