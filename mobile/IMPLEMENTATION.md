...
`workspace/AnalysisWorkspaceController` connects `MetricPreferenceDefault`, saved overrides, `SettlementAnalysisService`, and `UserPreferencesRepository` into the active application flow. It also carries the current `SettlementCandidateScope`; an imported scope is persisted with the dataset-scoped user context and counts as an explicit bounded analysis constraint, so even a legacy dataset with no preferences may analyze that reviewed ID set without enabling the protected unbounded auto-scan. Dataset-scoped Favorites are loaded and mutated through `FavoriteSettlementRepository`: add/remove/clear, note updates, and explicit frozen-snapshot replacement persist in the application settings database without changing candidate scope or triggering ranked recalculation. Shared analysis keeps a lightweight stable-ID lookup of the complete eligible ranking before the visible result limit is applied, so an eligible Favorite can show its actual current rank/score/coverage and refresh its frozen snapshot even when it falls below the visible top-N list. Adding a ranked result captures the already-authoritative score/coverage, effective Required criteria, enabled Preferences, and contribution values; later current-analysis edits do not rewrite that snapshot automatically. `notebook/FavoriteNotebookExport.kt` builds deterministic portable export content for the whole current dataset-scoped notebook: a versioned JSON manifest and Markdown summary with stable IDs, saved/current display names, notes, and frozen snapshot criteria. Input changes schedule a 250 ms debounced recalculation and late stale generations cannot replace newer ranked results. The controller also exposes explicit generic preference-edit operations for enabled state, weight, target/limit thresholds, and reset-to-dataset-default while keeping persisted overrides sparse. Legacy packages with no enabled preference defaults and no effective hard/radius constraint do not trigger an automatic unbounded candidate scan; explicit refresh remains available through the compatibility SearchPane path.

`desktop/headless/DesktopHeadlessWorkspace` is a developer/test composition adapter over that same controller. It wires the real Desktop JDBC/settings/Favorites repositories against explicit dataset/settings paths, waits for controller analysis and persistence jobs deterministically, and exposes headless operations for import, Required/Preferences, Favorites/note/snapshot, Favorites-to-import, batch-search planning, and ZIP export. `HeadlessCli.kt` is a thin command-line adapter over this surface; no scoring or persistence semantics are reimplemented there.
...
The shared application settings schema is version 7. Version 3 added `user_preferences.preferences_json`; version 4 added `candidate_scope_json`; version 5 added normalized `favorite_settlement` rows keyed by `(dataset_id, settlement_id)`; version 6 added nullable `note_text` plus `favorite_analysis_snapshot`; version 7 adds nullable `external_search_provider.query_terms_override`. Null is a backward-compatible unconfigured state that resolves to dataset search terms, an empty string intentionally adds no extra terms, and non-empty text is the provider-specific extra text. Shared settings UI presents the effective text directly and saves explicit values; **Set defaults** copies the dataset terms into all visible fields. Candidate-source payload version 2 still stores active source plus retained reviewed IDs/source text in the existing column. Favorites use a separate table and therefore remain independent from candidate-source filtering. The Desktop UI may explicitly bridge selected available Favorite IDs back into the existing imported candidate-source contract; this action does not perform alias resolution and preserves current Required/Preferences/center/radius inputs. The schema remains application-owned without changing `geo-format/VERSION`.
...
### `DesktopFavoriteNotebookExporter`

Shows a Desktop save dialog for `osmapdigger-favorites.zip`, writes the shared export bundle through a fixed-order STORED ZIP writer with stable timestamps/CRC metadata, publishes through a same-directory temporary file, and then reveals the saved file/location when the Desktop API supports it. Export ZIP creation never reads dataset/PBF paths or application settings.
...
Owns the document picker and current `AndroidDataset` lifecycle, then hosts the shared `OsmapDiggerApp`. It also injects `AndroidFavoriteNotebookExporter`; that adapter writes the shared export bundle under app cache, exposes only the export subdirectory through `FileProvider`, grants temporary read permission, and launches the standard Android share chooser. The polished Favorites browsing surface remains Desktop-first.
...
- dataset-scoped Favorites are persisted and browseable/selectable on Desktop, including notes, one frozen versioned analysis snapshot with explicit refresh, selected-Favorites re-analysis through the imported candidate source, and whole-notebook portable export. Checkbox multi-selection is independent from the current map/details Favorite: clicking a Favorite's analytical content selects/highlights it and focuses the map without closing the notebook, while **Open** returns to the normal details workflow. Named saved searches, multiple historical snapshots, and comparison state are not implemented yet.
