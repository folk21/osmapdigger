...
- dataset-scoped Favorites are persisted and browseable/selectable on Desktop, including notes, one frozen versioned analysis snapshot with explicit refresh, and selected-Favorites re-analysis through the imported candidate source; named saved searches, multiple historical snapshots, and comparison state are not implemented yet.
